...
class main_window(QWidget, Ui_Form):
    ...
    def Calculate(self):
        ...
        self.Steam.calc()
        state=self.Steam
        self.le_P.setText("{:.2f".format(self.Steam.P))  #set the text in each line edit and the label should tell the state 'saturated' or 'superheated'
        self.le_T.setText("{:.2f".format(self.Steam.T))
        self.le_Q.setText("{:.2f".format(self.Steam.x))
        self.le_H.setText("{:.2f".format(self.Steam.h))
        self.le_S.setText("{:.2f".format(self.Steam.s))
        self.le_SpV.setText("{:.2f".format(self.Steam.v))
